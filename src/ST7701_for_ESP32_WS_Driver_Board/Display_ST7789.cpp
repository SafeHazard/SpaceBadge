...
{ 
  LCDspi.beginTransaction(SPISettings(SPIFreq, MSBFIRST, SPI_MODE3)); //was SPI_MODE0 - AI suggested mode 3
...
//{
//    digitalWrite(5, HIGH);
//   ledcSetup(LCD_Backlight_PIN, Frequency, Resolution);   
...
void Backlight_Init()
{
    ...
    Set_Backlight(LCD_Backlight); // sets desired backlight level      //0~100                 
}

void Set_Backlight(uint8_t Light)
{
    if (Light > Backlight_MAX || Light < 0)
        LV_LOG_ERROR("Set Backlight parameters in the range of 0 to 100"); 
    else 
    { 
        uint32_t pwmValue = Light * 10; 
        if (pwmValue == 1000) 
            pwmValue = 1024; 
...
void LCD_SetRotation(uint8_t rot)
{
    LCD_WriteCommand(0x36);
    switch (rot % 4) {
    case 0: LCD_WriteData(0x00); break; // 0�
    case 1: LCD_WriteData(0x60); break; // 90�
    case 2: LCD_WriteData(0xC0); break; // 180�
    case 3: LCD_WriteData(0xA0); break; // 270�
    }
}
