...
bool Touch_I2C_Read(uint8_t Driver_addr, uint16_t Reg_addr, uint8_t *Reg_data, uint32_t Length)
{
  ...
  if ( Wire1.endTransmission(true)){
    LV_LOG_ERROR("The I2C transmission fails. - Touch I2C Read\r\n");
    return -1;
  }
...
bool Touch_I2C_Write(uint8_t Driver_addr, uint16_t Reg_addr, const uint8_t *Reg_data, uint32_t Length)
{
  ...
  if ( Wire1.endTransmission(true))
  {
    LV_LOG_ERROR("The I2C transmission fails. - Touch I2C Write\r\n");
    return -1;
  }
  return 0;
}
uint8_t Touch_Init(void) {
  ...
  LV_LOG_ERROR("Touch initialization failed!\r\n");
...
uint16_t CST328_Read_cfg(void) {
  ...
  LV_LOG_TRACE("TouchPad_ID:0x%02x,0x%02x,0x%02x,0x%02x\r\n", buf[0], buf[1], buf[2], buf[3]);
  Touch_I2C_Read(CST328_ADDR, HYN_REG_MUT_DEBUG_INFO_BOOT_TIME, buf, 4);
  LV_LOG_INFO("TouchPad_X_MAX:%d    TouchPad_Y_MAX:%d \r\n", buf[1]*256+buf[0],buf[3]*256+buf[2]);

  Touch_I2C_Read(CST328_ADDR, HYN_REG_MUT_DEBUG_INFO_TP_NTX, buf, 24);
  LV_LOG_TRACE("D1F4:0x%02x,0x%02x,0x%02x,0x%02x\r\n", buf[0], buf[1], buf[2], buf[3]);
  LV_LOG_TRACE("D1F8:0x%02x,0x%02x,0x%02x,0x%02x\r\n", buf[4], buf[5], buf[6], buf[7]);
  LV_LOG_TRACE("D1FC:0x%02x,0x%02x,0x%02x,0x%02x\r\n", buf[8], buf[9], buf[10], buf[11]);
  LV_LOG_TRACE("D200:0x%02x,0x%02x,0x%02x,0x%02x\r\n", buf[12], buf[13], buf[14], buf[15]);
  LV_LOG_TRACE("D204:0x%02x,0x%02x,0x%02x,0x%02x\r\n", buf[16], buf[17], buf[18], buf[19]);
  LV_LOG_TRACE("D208:0x%02x,0x%02x,0x%02x,0x%02x\r\n", buf[20], buf[21], buf[22], buf[23]);
  LV_LOG_TRACE("CACA Read:0x%04x\r\n", (((uint16_t)buf[11] << 8) | buf[10]));
